...
// are exposed separately for self-service flows driven from cinatoken's own UI.
...
func WaffoPancakeBuyerIdentityFromUserID(userID int) string {
	return fmt.Sprintf("cinatoken-user-%d", userID)
}
...
const (
	defaultWaffoPancakeStoreName   = "cinatoken-store"
	defaultWaffoPancakeProductName = "cinatoken-charge-product"
)
...
//
// OnetimeProduct (not SubscriptionProduct) because cinatoken has no renewal-
// event handling; Pancake auto-renewing without cinatoken extending user
